...
local CONNECTION_TIMEOUT = 10  -- Reduced from 15 for faster failure detection
local HANDSHAKE_TIMEOUT = 10  -- Reduced from 15 for faster failure detection
...
local function buildRequest(method, parsedUrl, requestHeaders, body)
    ...
    -- Apply request headers (these override any defaults)
...
local function processRequest(request)
    ...
    elseif request.state == REQUEST_STATE.CONNECTING then
        ...
        if not request.connectAttempted then
            ...
            elseif err == 'timeout' then
                -- Connection pending - check immediately if ready (don't wait for next poll)
                local _, writable, selectErr = socket.select(nil, { request.socket }, 0)
                if not selectErr and writable and #writable > 0 then
                    -- Socket is ready, complete connection immediately
                    completeConnect(request)
                    return
                end
    ...
    elseif request.state == REQUEST_STATE.SENDING_REQUEST then
        ...
        pcall(function() request.socket:settimeout(0.01) end) -- Very short timeout (10ms) for faster processing
...
function nonBlockingRequests.patch(url, body, headers, callback, options)
    return enqueueRequest('PATCH', url, headers, body, callback, options)
end

function nonBlockingRequests.delete(url, headers, callback, options)
    return enqueueRequest('DELETE', url, headers, nil, callback, options)
end
